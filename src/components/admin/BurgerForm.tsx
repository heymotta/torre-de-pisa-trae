...
interface BurgerFormProps {
  burger?: BurgerItem;
  onSubmit: (burger: BurgerItem) => void;
  onCancel: () => void;
}

export const BurgerForm = ({ burger, onSubmit, onCancel }: BurgerFormProps) => {
  const [formData, setFormData] = useState<Omit<BurgerItem, 'id'>>({
    name: '',
    description: '',
    price: 0,
    image: '',
  ...
  // Initialize form with burger data if editing
  useEffect(() => {
    if (burger) {
      setFormData({
        name: burger.name,
      ...
      setPriceInput(burger.price.toString().replace('.', ','));
    }
  }, [burger]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (burger) {
      ...
      // Add new burger with a temporary ID, actual ID will be set by the parent component
      onSubmit({
        ...formData,
        id: 'temp-id'
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 pt-4">
      ...
      <div className="space-y-2">
        ...
        {formData.image && (
          <div className="mt-2 relative h-40 w-full rounded-md overflow-hidden border border-motta-200">
            <img
              src={formData.image}
              alt="Preview"
      ...
      <DialogFooter>
        ...
        <Button type="submit">
          {burger ? 'Salvar Alterações' : 'Adicionar Hambúrguer'}
        </Button>
      </DialogFooter>
    </form>
  );
